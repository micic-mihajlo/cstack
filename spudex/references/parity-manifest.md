...
| Upstream | Spudex | Inventory treatment |
|---|---|---|
...
| `agents/poteto-agent.md` | `agents/spudex-agent.md`, `agents/templates/spudex-agent.toml` | Codex role prompt and optional custom-agent template. |
...
All 21 upstream principle leaves are present under `references/principles/`. Each is directly addressable through the single installed front door as `$spudex principle-<filename-stem>`, and `SKILL.md` maps that exact command to the leaf. They are not 21 independently installed Codex skills.

All 23 upstream support skills are present under `references/capabilities/`. Each is directly addressable as `$spudex <capability-directory-name>`, and `SKILL.md` maps that exact command to the module's `SKILL.md`. They are not 23 independently installed Codex skills. A separately packaged Codex plugin would need its own installation and validation before claiming independent discovery.
...
## Benny inventory and trust translation

The upstream Benny inventory is present under `automations/benny/`: two operational skills, their supporting references, three templates, `README.md`, and `FOR_AGENTS.md`. Cursor event triggers translate to two dormant Codex cron-automation specifications with idempotent polling.

The Codex translation adds required trust boundaries. External artifacts remain untrusted data. Writes require side-effect-free capability validation and renewable fenced leases. Linked code requires an allowlisted repository, immutable SHA, and trusted author or exact human approval. Build and app processes run in a fresh credential-free sandbox with no host mounts and denied-by-default egress. These files do not prove that a connector, sandbox, state adapter, automation, or end-to-end run exists. Benny remains dormant until the user configures and authorizes it.

## Guide and media inventory

All ten guide chapters, the guide index, and all six image assets are present. The former `02-poteto-mode.md` maps to `02-spudex.md`. Image-byte identity is a separate hash check. The guide text is a Codex translation and requires semantic review rather than filename comparison.
...
| Official `quick_validate.py <spudex-root>` | Root skill frontmatter and package-shape checks implemented by the current Codex skill validator. | Nested behavior, command execution, or upstream parity. |
...
- Cursor mode invocation becomes one installed `$spudex` front door with explicit nested command routing.
