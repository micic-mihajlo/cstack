...
|---|---|
| `/poteto-mode` | Explicit `$spudex` invocation. Natural-language requests can trigger Spudex through its description. |
| Independently listed nested pstack skills | This installation has one Codex skill front door. `$spudex <capability>` loads the exact capability module and `$spudex principle-<slug>` loads the exact principle leaf. The root `SKILL.md` contains and validates every mapping. Do not claim separate installed-skill discovery unless a Codex plugin that registers those skills is actually installed and verified. |
| Sticky mode | Codex has no identical per-chat sticky skill flag. Keep Spudex active for the current task and resumed work. For durable repository defaults, offer an `AGENTS.md` rule only when the user asks. For personal distribution, package Spudex as a plugin. |
...
## Automations and Benny

The Benny pack is dormant source material. In the Codex app, create or update its two automations through the automation tool only after the user explicitly asks. Preserve the app's notification policy separately from the prompt.

Connectors must be present and authorized. If Slack, the issue tracker, repository, or control adapter is missing, say exactly which access is absent and fail closed for external writes.

Benny validates every required capability without side effects before its first write. Its no-write triage phase produces separate evidence, decision, and write-plan records. Live writers use renewable fenced leases whose TTL exceeds the maximum run. Linked code must resolve to an allowlisted repository and immutable SHA with a trusted author or exact human approval, then run in a fresh credential-free sandbox with no host mounts and denied-by-default egress.
