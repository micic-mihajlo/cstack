...
In the Codex app, use task and thread tools as the primary history source. In CLI contexts, use the current transcript or explicit handoff material. Do not guess at unrelated private transcript locations.
...
2. Lock the scope before searching. Pin the window. Treat "recent" as the last seven days unless context gives a better range. Pin the topic and workspace. Never read another project's tasks or files without a relevant request. State the scope. Never quietly turn "all" into a smaller window.
3. Fan out across named Codex tasks when volume justifies it. Resolve candidate tasks with `list_threads`, then give each read-only lane exact task IDs and use `read_thread`. For a user-provided transcript, divide explicit file ranges. Each lane returns one block per task with topic, user goal, decisions, open threads, corrections, and artifacts. Cite the task title and ID or explicit transcript path. For one or two tasks, read directly. Do not search hidden session stores, use filesystem modification time as task ordering, or read unrelated private chats.
...
5. Verify against live state. A task summary or stale ticket is history, not current truth. Refresh surfaced PRs, branches, tickets, and provider state through their real read surfaces. When the answer hinges on what an agent actually did, read the named Codex task with outputs enabled or inspect its explicit evidence artifacts. State any visibility gap.
