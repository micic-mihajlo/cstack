---
name: make-bot-ui
description: Build a secure page or dashboard that starts and resumes a real Codex SDK, codex exec, or explicitly experimental app-server workflow. Use for $spudex make-bot-ui, expose this agent as a UI, or put buttons in front of a Codex routine.
---
...
- Accept state changes only through `POST`, `PUT`, `PATCH`, or `DELETE` with `Content-Type: application/json`, a bounded body, a strict schema, and a required custom header such as `X-Spudex-Request: 1`. Reject form, text, multipart, and simple cross-origin requests unless a separately reviewed upload endpoint requires one exact media type.
