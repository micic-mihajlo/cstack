...
6. Run `node <spudex-root>/scripts/check-plan.mjs <plan.md>` and fix every reported line. Resolve `<spudex-root>` to this skill's installed directory. The validator enforces the skeleton, verification rule, and writing constraints.
...
The program runs `<spudex-root>/playbooks/<execution playbook>.md`. <Who merges, and which proposed PR ids are the operator's items that stop at merge-ready.>
...
- [ ] Read these from the current installed Spudex bundle at program start. Re-read them at every wake.
  - [ ] `<spudex-root>/playbooks/<execution playbook>.md`
  - [ ] `<spudex-root>/references/capabilities/swarm/SKILL.md`
  - [ ] `<spudex-root>/references/codex-runtime.md`
  - [ ] `<spudex-root>/playbooks/opening-a-pr.md`
  - [ ] `<spudex-root>/references/principles/<each applied leaf>.md`
...
- [ ] Run `$unslop` when available plus an exact-diff self-review before each commit. Apply Spudex No comments before review.
...
- [ ] Save every screenshot to `/tmp/spudex-swarm-<pr-id>/worker-<n>/<slug>.png` and return the paths with the report.
...
<Docs to read before editing. Which PRs use Spudex How and Interrogate. The decision trail follows Show me your work.>
````
