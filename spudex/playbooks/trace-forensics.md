...
1. Identify the format and load it with the right tool for that format. When tool policy permits and the artifact is too large for the main context, delegate parsing and keep the reduced finding in the main thread. Do not inline a multi-megabyte trace.
2. Transform the raw artifact into a queryable form before analyzing it. Use SQLite when rows and aggregates fit the format. Use the native profiler or a purpose-built parser when it preserves relationships better.
