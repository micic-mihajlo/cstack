...
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. When tool policy permits and the artifact is too large for the main context, delegate parsing and keep the reduced finding in the main thread.
3. Prove the mechanism before believing it. Use read-only instrumentation, a local reproduction, or an isolated temporary patch. Do not mutate production during a diagnosis. A plausible but unconfirmed cause may be wrong while the real cause sits one layer over.
