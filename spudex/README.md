...
```text
~/.agents/skills/spudex/
```

Repository-shared installations belong at `.agents/skills/spudex/`. This machine's working copy is intentionally still at `~/.codex/skills/spudex/`, the path supplied by its owner and currently loaded by Codex; do not move or duplicate it as part of an in-place update.

Restart Codex or start a fresh task, then confirm Spudex appears in the skill list. Invoke it explicitly with `$spudex`. Its description also supports implicit routing for non-trivial engineering work.

For a team distribution with multiple independently installed skills or MCP dependencies, package and validate the bundle as a Codex plugin. This installation has one installed front door. Nested principles and capabilities do not appear as separate installed skills. Invoke them as `$spudex principle-<slug>` or `$spudex <capability>`; the root skill maps that exact command and loads the matching module in full. `agents/openai.yaml` is Codex skill UI metadata; it is not a custom-agent definition. Persistent custom agents use standalone TOML files under `~/.codex/agents/` or `.codex/agents/`.

## Get started

1. Run `$spudex setup-spudex` only if you want persistent agent or model-role configuration.
2. Use `$spudex` for engineering work that needs rigor.

```text
$spudex this PR has a subtle bug where scrolling drifts while idle. reproduce it first, then fix and verify.
```

```text
$spudex I am stepping away. migrate every caller in an isolated worktree. done means zero old callers, the old API is deleted, and the real command passes. keep a decision log.
```
...
Codex has no identical sticky-mode flag. Spudex remains the operating contract for the current task and resumed work. Say so when you want to opt out. See [Codex runtime](references/codex-runtime.md) for every product translation.
...
Spudex routes to these modules as needed. You can also ask for one directly as `$spudex <name>`. This is command routing through the single installed Spudex skill, not independent skill installation.
...
| [setup-spudex](references/capabilities/setup-spudex/SKILL.md) | Configure validated Codex agents and role models. |
...
[Spudex agent](agents/spudex-agent.md) and [Comment Sicko](agents/comment-sicko.md) are bundled role prompts. Matching installable TOML templates live under [agents/templates](agents/templates). The root mode resumes an existing `spudex-agent` thread instead of starting a sibling. Codex persistent custom agents are TOML files under `~/.codex/agents/` or `.codex/agents/`; `$spudex setup-spudex` installs them only when the user asks for persistent agents and a built-in role cannot express the requested behavior. Comment Sicko's template requests `sandbox_mode = "read-only"`, and setup must verify that the spawned child is effectively read-only because a live parent permission override can supersede that default.
...
Invoke any leaf directly as `$spudex principle-<slug>`. The root mapping loads that leaf in full. These leaves are not registered as 21 separate Codex skills in this installation.
...
Codex already has plan mode and an active plan surface. Spudex uses them when the work benefits from a durable checklist. Small obvious changes skip ceremony. The [multi-phase playbook](playbooks/multi-phase-plan.md) produces a plan and stops unless the user separately asks for execution.

## Make it yours

Use `$spudex automate-me` to draft a personal mode from the current task and explicitly scoped prior tasks. Use `$spudex setup-spudex` to configure role models. Both preserve unrelated configuration and write persistently only when asked.

## Automations

The dormant [Benny automation pack](automations/benny/README.md) triages issue reports, then reproduces and fixes confirmed bugs with real UI evidence. The local-project scheduled tasks Benny uses in the desktop app are time-based, so the port uses idempotent polling and atomic claims. Eligible web and mobile plans can trigger scheduled tasks from Slack events, but those event-triggered tasks are unavailable in the desktop app, CLI, and IDE and cannot provide Benny's local-project execution surface. Benny creates no automation until the user asks.
