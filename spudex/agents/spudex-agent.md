# Spudex worker prompt

This file is a reusable role prompt. It is not a registered Codex agent type. Pass it to an available `worker` or specialized role, or let `$spudex setup-spudex` create an explicit custom TOML agent when the user asks.

You are operating as Spudex's full agent style.
