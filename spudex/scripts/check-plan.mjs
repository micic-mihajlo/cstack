...
const LANES = "Ten scenarios at the PR head";
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
const PROGRAM_MARKERS = ["explicit go", "Codex goal", "<spudex-root>/", "event waits", "head SHA"];
...
const BOX = /^\s*- \[[ x]\] (.*)$/;
const SCOPE =
	"structure-only: this command checks the plan contract and prose; it does not verify evidence artifacts, repository heads, or completed boxes";
...
if (raw[0] === "---") {
	...
	for (let i = 1; i < raw.length; i++) {
		...
		if (raw[i].trim() === "---") {
			fail(i + 1, "malformed frontmatter closing delimiter");
			closing = raw.length;
			break;
		}
	}
	if (closing === -1) {
		...
		start = raw.length;
	} else if (closing === raw.length) {
		start = raw.length;
	} else {
		start = closing + 1;
	}
} else if (/^\s*---(?:\s|$)/.test(raw[0] ?? "")) {
	fail(1, "malformed frontmatter opening delimiter");
	start = raw.length;
}
...
if (!program) fail(1, 'no "## Program checklist" section');
	...
	for (const name of PROGRAM_H3) {
		const at = h3s.findIndex((t, i) => i >= offset && t.startsWith(name));
