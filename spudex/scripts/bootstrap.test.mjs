...
test("bounds a real child by elapsed time and captured output", () => {
  ...
  const credentialName = "SPUDEX_BOOTSTRAP_TEST_CREDENTIAL";
...
test("fails closed on a stale lock and serializes two real installers", async (t) => {
  const root = mkdtempSync(join(temporaryDirectory, "spudex-bootstrap-"));
  ...
  const lock = join(root, ".spudex-tools-install.lock");
  writeFileSync(lock, "99999999\n", { mode: 0o600 });
  chmodSync(lock, 0o600);
  const stale = await launch(root);
  assert.notEqual(stale.code, 0);
  assert.match(stale.stderr, /stale Spudex dependency-install lock for dead PID 99999999/);
  ...
  assert.deepEqual(
    readdirSync(root).filter((name) =>
      name.startsWith(".spudex-tools-stage-") ||
      name.startsWith(".spudex-tools-backup-")
    ),
    []
  );
  const marker = join(root, "node_modules", ".spudex-tools-install-key");
  const integrity = join(root, "node_modules", ".spudex-tools-integrity.json");
