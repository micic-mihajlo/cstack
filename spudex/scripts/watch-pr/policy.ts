import {
  ChecksUnavailable,
  WatcherQueryError,
  assertStackContextsCurrent,
} from "./github.ts";
import type * as T from "./types.ts";
import { nonEmpty } from "./types.ts";
export function assessGitHubMerge(args: {
  readonly mergeStateStatus: T.MergeStateStatus;
  readonly headGateState: "FAILURE" | "PENDING" | "SUCCESS";
  readonly draftAllowed: boolean;
}): T.GitHubMergeAssessment {
  const mergeStateAllowed =
    args.mergeStateStatus === "CLEAN" ||
    (args.mergeStateStatus === "DRAFT" && args.draftAllowed);
  if (
    mergeStateAllowed &&
    args.headGateState === "SUCCESS"
  )
    return {
      kind: "allowed",
      basis: "merge-state-and-rest-gates",
      mergeStateStatus: args.mergeStateStatus,
      headGateState: args.headGateState,
    };
  return {
    kind: "refused",
    reason:
      mergeStateAllowed ? "head-gates" : "merge-state",
    mergeStateStatus: args.mergeStateStatus,
    headGateState: args.headGateState,
  };
}
async function mergeAssessment(
  ...
  checks: T.CheckRead,
  includePendingHistory: boolean,
  allowDraft: boolean
) {
  const commits = includePendingHistory
    ? await reader.commitRollups(facts.context)
    : [];
  const headGateState = checks.checks.some((check) => check.kind === "failed")
    ? "FAILURE"
    : checks.checks.some((check) => check.kind === "pending")
      ? "PENDING"
      : "SUCCESS";
  return {
    ...
    github: assessGitHubMerge({
      ...
      headGateState,
      draftAllowed: facts.isDraft && allowDraft,
    }),
  };
}
...
export type SnapshotFactsPlan = "merged" | "closed" | "read-open-details";
export function planSnapshotAfterFacts(
  facts: T.PullRequestFacts
): SnapshotFactsPlan {
  if (facts.state === "MERGED" || facts.mergedAt !== null) return "merged";
  return facts.state === "CLOSED" ? "closed" : "read-open-details";
}
export function needsMergeAssessment(args: {
  ...
  return !(
    args.failedCount === 0 &&
    args.pendingCount > 0 &&
    args.pendingHistory === "omit"
  );
}
export interface MergeEvidence {
  readonly hadPreviousPassingCi: boolean;
  readonly github: T.GitHubMergeAssessment;
}
export type CiResolution =
  | { readonly kind: "needs-merge-evidence" }
  | { readonly kind: "complete"; readonly ci: T.CiState };
export function resolveCiState(args: {
  readonly checks: T.CheckRead;
  readonly pendingHistory: "include" | "omit";
  readonly merge?: MergeEvidence;
}): CiResolution {
  const failed = nonEmpty(
    args.checks.checks.filter(
  ...
  if (
    !needsMergeAssessment({
      failedCount: failed?.length ?? 0,
      pendingCount: pending?.length ?? 0,
      pendingHistory: args.pendingHistory,
    })
  ) {
    if (pending === null)
      throw new Error("pending CI plan lost its pending check");
    return {
      kind: "complete",
      ci: {
        kind: "ci-pending",
        source: args.checks.source,
        headRefOid: args.checks.headRefOid,
        checkRunCount: args.checks.checkRunCount,
        statusCount: args.checks.statusCount,
        all: args.checks.checks,
        failed: [],
        pending,
  ...
  if (args.merge === undefined) return { kind: "needs-merge-evidence" };
  const base = {
    source: args.checks.source,
    headRefOid: args.checks.headRefOid,
    checkRunCount: args.checks.checkRunCount,
    statusCount: args.checks.statusCount,
    all: args.checks.checks,
    hadPreviousPassingCi: args.merge.hadPreviousPassingCi,
  };
  if (failed !== null)
    return {
      kind: "complete",
  ...
  if (args.merge.github.kind === "refused")
    return {
      kind: "complete",
      ci: {
        ...base,
...
export interface OpenSnapshotSample {
  readonly threads: readonly T.ReviewThread[];
  readonly ci: T.CiState;
  readonly reviewAutomationRunning: boolean;
}
async function collectOpenSnapshotSample(args: {
  readonly reader: T.GitHubReader;
  readonly facts: T.PullRequestFacts;
  readonly pendingHistory: "include" | "omit";
  readonly allowDraft: boolean;
}): Promise<OpenSnapshotSample> {
  const threads = await args.reader.reviewThreads(args.facts.context);
  if (args.facts.headRefOid === null)
    throw new ChecksUnavailable(
      "GitHub did not report an exact pull request head SHA"
    );
  const checks = await args.reader.exactHeadChecks(
    args.facts.context,
    args.facts.headRefOid
  );
  let resolved = resolveCiState({ checks, pendingHistory: args.pendingHistory });
  if (resolved.kind === "needs-merge-evidence")
    resolved = resolveCiState({
      checks,
      pendingHistory: args.pendingHistory,
      merge: await mergeAssessment(
        args.reader,
        args.facts,
        checks,
        args.pendingHistory === "include" &&
          checks.checks.some((check) => check.kind === "pending"),
        args.allowDraft
      ),
    });
  if (resolved.kind !== "complete")
    throw new Error("CI resolution still needs merge evidence after query");
  return {
    threads,
    ci: resolved.ci,
...
const checkFingerprint = (check: T.Check): string =>
  JSON.stringify([
    check.kind,
    check.name,
    check.reportedState,
...
function stabilityFingerprint(
  facts: T.PullRequestFacts,
  sample: OpenSnapshotSample
): string {
  const normalizedChecks = [...sample.ci.all]
    .map(checkFingerprint)
    .sort();
  const normalizedThreads = [...sample.threads]
    .map((thread) => JSON.stringify(thread))
    .sort();
  const github =
    sample.ci.kind === "ci-clean" ||
    sample.ci.kind === "ci-failing" ||
    sample.ci.kind === "ci-github-rejected"
      ? sample.ci.github
      : null;
  return JSON.stringify({
    ...
    ci: {
      kind: sample.ci.kind,
      source: sample.ci.source,
      headRefOid: sample.ci.headRefOid,
      checkRunCount: sample.ci.checkRunCount,
      statusCount: sample.ci.statusCount,
      checks: normalizedChecks,
      hadPreviousPassingCi: sample.ci.hadPreviousPassingCi,
      github,
    },
    reviewAutomationRunning: sample.reviewAutomationRunning,
  });
}
export function openObservationsAgree(
  firstFacts: T.PullRequestFacts,
  firstSample: OpenSnapshotSample,
  secondFacts: T.PullRequestFacts,
  secondSample: OpenSnapshotSample
): boolean {
  return (
    stabilityFingerprint(firstFacts, firstSample) ===
    stabilityFingerprint(secondFacts, secondSample)
  );
}
function unstableSnapshot(): never {
  throw new WatcherQueryError({
    kind: "missing-key",
    retryable: true,
    detail:
      "pull request changed while readiness was sampled; retrying until two complete observations agree",
  });
}
export async function readSnapshot(args: {
  ...
  const firstFacts = await args.reader.pullRequest(args.context);
  const factsPlan = planSnapshotAfterFacts(firstFacts);
  if (factsPlan === "merged")
    return { kind: "merged", context: args.context, facts: firstFacts };
  if (factsPlan === "closed")
    return { kind: "closed", context: args.context, facts: firstFacts };
  const firstSample = await collectOpenSnapshotSample({
    reader: args.reader,
    facts: firstFacts,
    pendingHistory: args.pendingHistory,
    allowDraft: args.allowDraft,
  });
  const secondFacts = await args.reader.pullRequest(args.context);
  if (planSnapshotAfterFacts(secondFacts) !== "read-open-details")
    unstableSnapshot();
  const secondSample = await collectOpenSnapshotSample({
    reader: args.reader,
    facts: secondFacts,
    pendingHistory: args.pendingHistory,
    allowDraft: args.allowDraft,
  ...
  return {
    ...
    facts: finalFacts,
    threads: secondSample.threads,
    ci: secondSample.ci,
    reviewAutomationRunning: secondSample.reviewAutomationRunning,
  };
}
...
function gateReason(
  ...
  if (row.kind === "merged")
    return row.facts.headRefOid === null ? "head-sha-unavailable" : null;
  if (row.kind === "closed") return "closed-without-merge";
  if (row.facts.isDraft && !allowDraft) return "draft-pr";
  if (row.facts.headRefOid === null) return "head-sha-unavailable";
...
function readyContribution(
  ...
  if (row.kind === "merged") {
    if (row.facts.headRefOid === null) return null;
    return {
      kind: "merged-pr",
      context: row.context,
  ...
  if (
    ...
    return null;
  const headRefOid = row.facts.headRefOid;
...
export interface RunDependencies {
  ...
  /** Start of the whole operation, including context and stack discovery. */
...
const deadlineVerdict = (stamp: VerdictStamp): T.TimeoutVerdict =>
  stamp({
    kind: "TIMEOUT",
    terminal: true,
    exitCode: 5,
    reason: { kind: "deadline" },
  });
async function pollUntilTerminal<V>(args: {
  ...
  let failures = 0;
  const started = args.dependencies.startedAt ?? args.dependencies.clock.now();
  let timeoutFromLastWait: (() => V) | undefined;
  while (true) {
    if (deadlinePassed(started, args.options, args.dependencies.clock.now()))
      return timeoutFromLastWait?.() ?? deadlineVerdict(args.stamp);
...
export async function runSimple(args: {
  ...
  const step = async (): Promise<StepResult<T.TerminalVerdict>> => {
    ...
    if (
      ...
      if (active !== null)
        await assertStackContextsCurrent(args.dependencies.reader, active);
    }
...
export function evaluateQueue(
  ...
  const missingMergedHead = orderedRows(state).find(
    (row) => row.kind === "merged" && row.facts.headRefOid === null
  );
  if (missingMergedHead !== undefined)
    return {
      kind: "blocker",
      state,
      blocker: {
        kind: "merge-gate",
        pr: missingMergedHead.context,
        reason: "head-sha-unavailable",
      },
    };
  ...
  if (active.length === 0) {
    const merged = nonEmpty(
      orderedRows(state).flatMap((row) =>
        row.kind === "merged" && row.facts.headRefOid !== null
          ? [
              {
...
export const queueCadence = (
  evaluation: Pick<QueueEvaluation, "kind">
): "continue" | "sleep" | null =>
  evaluation.kind === "advance"
    ? "continue"
    : evaluation.kind === "waiting"
      ? "sleep"
      : null;
export function queueReadContext(state: QueueState): T.PrContext {
  if (state.work === null) throw new Error("queue has no read in flight");
  return state.work.kind === "whole-stack-sweep"
...
export async function runQueued(args: {
  ...
  let state = createQueueState(
    args.contexts,
    args.dependencies.startedAt ?? args.dependencies.clock.now()
  );
  ...
  const step = async (): Promise<StepResult<T.QueueTerminalVerdict>> => {
    ...
    const context = queueReadContext(state);
    ...
    switch (evaluation.kind) {
      ...
      case "advance":
        ...
        if (queueCadence(evaluation) !== "continue")
          throw new Error("advance must continue without sleeping");
      ...
      case "waiting":
        ...
        if (queueCadence(evaluation) !== "sleep")
          throw new Error("waiting must sleep before polling again");
