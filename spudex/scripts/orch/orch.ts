...
import {
  ...
  UsageError,
  openStore,
  parseConstraintAuthorityCeiling,
  parseConstraintProvenance,
  parseVerdict,
  writeVerificationReceipt,
  type ConstraintAuthorityCeiling,
...
const MAX_RENDERED_OUTPUT_BYTES = 1024 * 1024;
...
interface LedgerGateOptions {
  readonly repo: string;
  readonly allowTypeCheckOnly: boolean;
}

interface LedgerLookupOptions {
  readonly repo: string;
}
...
interface StandingSupersedeOptions {
  readonly by: string;
}
...
function compactScalar(value: unknown): string {
  return terminalText(String(value)).replace(/[\t\r\n]/g, "?");
}

function compactLine(values: readonly unknown[]): string {
  return values.map(compactScalar).join("\t");
}
...
function receiptExitStatus(value: string): number | null {
  if (value === "none") {
    return null;
  }
  const parsed = Number(value);
  if (!/^\d{1,3}$/.test(value) || !Number.isSafeInteger(parsed) || parsed > 255) {
    throw new InvalidArgumentError("must be none or an integer from 0 to 255");
  }
  return parsed;
}
...
function drainLines(result: InboxDrainResult): string {
  ...
  return [
    ...
    compactRows(result.pointers, pointerLine, "(empty)", null),
  ].join("\n");
}
...
function constraintLine(record: ConstraintRecord): string {
  return compactLine([
    record.id,
    record.state.kind,
    record.provenance,
    record.scope,
    record.authorityCeiling,
    record.digest,
  ]);
}
  ...
  const prs =
    value.prs.length === 0
      ...
      : value.prs
          .map(
            (row) =>
              `${compactScalar(row.branches)}#${row.pr}@${row.sha}:${row.state}`
          )
          .join(",");
...
function statusLines(report: StatusReport): string {
  ...
  return [
    ...
    `gates open: ${report.summary.openGateIds.length}${
      visible.length > 0
        ? `; ids=${visible.map(compactScalar).join(",")}${more}`
        : ""
    }`,
  ].join("\n");
}
...
function emit<T>(
  ...
  const rendered = json
    ...
    : compact(value);
  if (Buffer.byteLength(rendered, "utf8") > MAX_RENDERED_OUTPUT_BYTES) {
    throw new UsageError(
      `output exceeds the ${MAX_RENDERED_OUTPUT_BYTES}-byte limit; use a filtered or batched command`
    );
  }
...
function createProgram(io: Io): Command {
  const program = new CommanderCommand("orch")
    ...
    .configureOutput({
      writeOut: io.stdout,
      writeErr: (value: string) => io.stderr(terminalText(value)),
    })
    .exitOverride()
    .showHelpAfterError()
    .allowExcessArguments(false)
    ...
    .option(
      "--force",
      "legacy compatibility flag; never overrides a live writer",
      false
    );
  ...
  leaf(program, "init", "initialize the store").action(() =>
    runStore(
      ...
      io,
      (store) => store.init(),
      (result) => `initialized ${compactScalar(result.store)}`
    )
  );
  ...
  const ledger = program
    ...
    .action(() => requireSubcommand(program));
  leaf(ledger, "receipt", "write a private coordinator verification receipt")
    .argument("<pr>", "pull request number", positiveInteger)
    .argument("<sha>", "complete commit SHA")
    ...
    .requiredOption("--command <command>", "exact verification command or procedure")
    .requiredOption("--surface <surface>", "real verification surface")
    .requiredOption(
      "--exit-status <code>",
      "none or integer exit status",
      receiptExitStatus
    )
    .requiredOption("--evidence <path>", "completed verifier evidence file")
    .requiredOption("--out <path>", "new write-once receipt path")
    .option("--verified-at <iso>", "canonical ISO verification timestamp")
    .action(
      async (
        pr: number,
        sha: string,
        ...
        emit(
          ...
          result,
          (value) => compactLine([value.path, value.receipt.digest])
        );
      }
    );
  leaf(ledger, "record", "record a coordinator-attested completion receipt")
    .argument("<pr>", "pull request number", positiveInteger)
    .argument("<sha>", "commit SHA")
    .argument("<verdict>", "verification verdict", parseVerdict)
    .requiredOption("--repo <host/owner/name>", "canonical repository identity")
    .requiredOption("--receipt <path>", "immutable coordinator receipt path")
    .action(
      (
        pr: number,
        sha: string,
  ...
  leaf(ledger, "check", "show a recorded verdict without gating completion")
    .argument("<pr>", "pull request number", positiveInteger)
    .argument("<sha>", "commit SHA")
    .requiredOption("--repo <host/owner/name>", "canonical repository identity")
    .action((pr: number, sha: string, options: LedgerLookupOptions) =>
      runStore(
        program,
        io,
        (store) =>
          store.ledger.check({ repository: options.repo, pr, sha }),
        (row) => row.verdict
      )
    );
  leaf(ledger, "gate", "apply completion policy to a coordinator-attested receipt")
    .argument("<pr>", "pull request number", positiveInteger)
    .argument("<sha>", "complete commit SHA")
    .requiredOption("--repo <host/owner/name>", "canonical repository identity")
    .option(
      "--allow-type-check-only",
      "allow type-check-only only when the completion contract permits it",
      false
    )
    .action((pr: number, sha: string, options: LedgerGateOptions) =>
      runStore(
        program,
        io,
        (store) =>
          store.ledger.gate({
            ...
            pr,
            sha,
            allowTypeCheckOnly: options.allowTypeCheckOnly,
          }),
        (row) => row.verdict
      )
    );
  ...
  leaf(inbox, "drain", "drain inbox pointers")
    ...
    .action(async (options: InboxDrainOptions) => {
      ...
      await runStore(
        program,
        io,
        (store) => store.inbox.drain(),
        drainLines
      );
    });
  leaf(inbox, "ack <batch>", "acknowledge and delete a claimed batch").action(
    (batch: string) =>
      runStore(
        program,
        io,
        (store) => store.inbox.ack(batch),
        (result) => compactLine([result.batch, result.count])
      )
  );
  ...
  leaf(gate, "park <id>", "park a decision gate")
    ...
    .action((id: string, options: GateParkOptions) =>
      runStore(
        ...
        (result) => compactLine([result.id, "open"])
      )
    );
  ...
  leaf(gate, "resolve <id>", "resolve a decision gate")
    ...
    .requiredOption("--source <user:pointer>", "coordinator-recorded user source")
    .action((id: string, options: GateResolveOptions) =>
      runStore(
        ...
        (result) => compactLine([result.id, "resolved", result.answer])
      )
    );

  const frontier = program
    .command("frontier")
    .description("manage the local Graphite topology snapshot")
    .action(() => requireSubcommand(program));
  leaf(
    frontier,
    "set",
    "record local Graphite topology; never use it as hosted merge proof"
  )
    .addOption(
      new Option(
        "--repo <dir>",
  ...
  leaf(standing, "show", "show standing orders").action(() =>
    runStore(
      ...
      (rows) => compactRows(rows, constraintLine, "(no standing orders)")
    )
  );
  leaf(standing, "add <id>", "add a provenance-bound constraint")
    .requiredOption("--constraint <text>", "normalized constraint text")
    .requiredOption(
      "--provenance <class>",
      "direct-user, repository-policy, or coordinator-safety",
      parseConstraintProvenance
    )
    .requiredOption("--source <pointer>", "trusted provenance pointer")
    .requiredOption("--scope <scope>", "normalized applicability scope")
    .requiredOption(
      "--authority-ceiling <ceiling>",
      "advisory, read-only, or local-write",
      parseConstraintAuthorityCeiling
    )
    .action((id: string, options: StandingAddOptions) =>
      runStore(
        program,
        io,
        (store) => store.standing.add({ id, ...options }),
        constraintLine
      )
    );
  leaf(standing, "supersede <id>", "supersede a constraint")
    .requiredOption("--by <id>", "active replacement constraint id")
    .action((id: string, options: StandingSupersedeOptions) =>
      runStore(
        program,
        io,
        (store) => store.standing.supersede({ id, by: options.by }),
        constraintLine
      )
    );
...
function handleError(error: unknown, program: Command, io: Io): number {
  ...
  if (error instanceof UsageError) {
    io.stderr(terminalText(program.helpInformation()));
  }
  return 1;
}
