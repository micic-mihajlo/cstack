...
const expectedLocalRequired = [
  ...
  "agents/openai.yaml",
  "agents/spudex-agent.md",
  "agents/comment-sicko.md",
  "agents/templates/spudex-agent.toml",
...
if (symbolicLinks.length) {
  process.stderr.write(
    `symbolic links are forbidden in the Spudex skill tree:\n${symbolicLinks
...
const capabilityCommands = [...rootSkill.matchAll(/`\$spudex ([a-z0-9-]+)`/g)]
  .map((match) => match[1])
  .filter((command) => !command.startsWith("principle-"));
for (const name of expectedCapabilities) {
  const exact = `- \`$spudex ${name}\` -> [${name}](references/capabilities/${name}/SKILL.md)`;
  if (rootSkill.split(exact).length !== 2) {
    fail(`SKILL.md must contain one canonical $spudex ${name} mapping`);
  }
  if (capabilityCommands.filter((command) => command === name).length !== 1) {
    fail(`SKILL.md must expose $spudex ${name} exactly once`);
  }
}
...
const principleCommands = [...rootSkill.matchAll(/`\$spudex principle-([a-z0-9-]+)`/g)].map(
  (match) => match[1]
);
for (const filename of expectedPrinciples) {
  ...
  const canonical = new RegExp(
    `^- \`\\$spudex principle-${escapedStem}\` -> \\[[^\\]]+\\]\\(references/principles/${escapedStem}\\.md\\)`,
    "gm"
  );
  if (commandCount !== 1 || [...rootSkill.matchAll(canonical)].length !== 1) {
    fail(`SKILL.md must contain one canonical $spudex principle-${stem} mapping`);
  }
}
...
if (!openai.includes("$spudex")) fail("agents/openai.yaml default prompt must mention $spudex");
...
for (const [path, name] of [
  ["agents/templates/spudex-agent.toml", "spudex-agent"],
...
if (packageJson.name !== "@spudex/tools") fail("scripts package name is not @spudex/tools");
...
const setupBenny = text(
  join(root, "automations", "benny", "skills", "setup-benny", "SKILL.md")
);
for (const logicalEntry of [
  ".codex/automations/benny/skills/triage-issue-reports/SKILL.md",
  ".codex/automations/benny/skills/reproduce-and-fix-issues/SKILL.md",
]) {
  const snapshotOnlyInstruction = `The task identifies \`${logicalEntry}\` as the logical pack entry. At run time it must read and follow only that entry from the digest-matched current-user-private snapshot, never from the live checkout.`;
  if (!setupBenny.includes(snapshotOnlyInstruction)) {
    fail(`setup-benny must bind ${logicalEntry} to the digest-matched private snapshot`);
  }
}
if (
  /(?:must read and follow|reads and follows) `\.codex\/automations\/benny\/skills\/[^`]+`/.test(
    setupBenny
  )
) {
  fail("setup-benny must not tell an automation to follow the live checkout");
}
...
if (!Array.isArray(fileMap.entries)) {
  ...
  for (const entry of fileMap.entries) {
    ...
    if (entry.treatment === "omitted-by-local-test-integrity-policy") {
    ...
    for (const mapped of entry.mappedLocalPaths) {
      ...
      if (absolute !== root && !absolute.startsWith(`${root}${sep}`)) {
        fail(`${entry.upstreamPath}: mapped local path escapes the Spudex root ${mapped}`);
        continue;
      }
      ...
      if (canonical !== canonicalRoot && !canonical.startsWith(`${canonicalRoot}${sep}`)) {
        fail(`${entry.upstreamPath}: mapped local file resolves outside the Spudex root ${mapped}`);
      }
    }
  ...
  const expectedOmitted = ["skills/poteto-mode/scripts/watch-pr/fakes.test-helper.ts"];
...
const driftPatterns = [
  ...
  [/skills\$spudex/, "malformed skill path"],
...
for (const path of files.filter((item) => extname(item) === ".md")) {
  ...
  for (const match of body.matchAll(pattern)) {
    ...
    if (linked !== root && !linked.startsWith(`${root}${sep}`)) {
      fail(`${rel(path)}: link escapes the Spudex root ${target}`);
      continue;
    }
    ...
    if (canonical !== canonicalRoot && !canonical.startsWith(`${canonicalRoot}${sep}`)) {
      fail(`${rel(path)}: link resolves outside the Spudex root ${target}`);
    }
  }
}
...
process.stdout.write(
  `Spudex validation passed: 23 playbooks, 21 principles, 23 capabilities, 6 guide images, ${files.length} files\n`
);
