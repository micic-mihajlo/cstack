...
test("audits a real worktree whose path contains spaces without fetching", (t) => {
  const root = mkdtempSync(join(temporaryDirectory, "spudex-worktree-audit-test-"));
  ...
  git(repository, "config", "user.name", "Spudex Test");
  git(repository, "config", "user.email", "spudex-test@example.invalid");
...
test("holds a worktree that contains only untracked scratch files", (t) => {
  const root = mkdtempSync(join(temporaryDirectory, "spudex-worktree-scratch-test-"));
  const repository = join(root, "repo");
  ...
  const result = spawnSync("bash", [script, repository], {
    encoding: "utf8",
    env: { ...process.env, SPUDEX_BASE_REF: "main" },
  });
  assert.equal(result.status, 0, result.stderr);
...
test("refuses a non-GitHub origin before an explicit fetch", (t) => {
  const root = mkdtempSync(join(temporaryDirectory, "spudex-worktree-origin-test-"));
  const repository = join(root, "repo");
  ...
  const result = spawnSync("bash", [script, "--fetch", repository], {
    encoding: "utf8",
    env: { ...process.env, SPUDEX_BASE_REF: "origin/main" },
  });
...
test("rejects executable local Git configuration before auditing", (t) => {
  const root = mkdtempSync(join(temporaryDirectory, "spudex-worktree-config-test-"));
  const repository = join(root, "repo");
  t.after(() => rmSync(root, { recursive: true, force: true }));
  git(root, "init", "-b", "main", repository);
...
test("rejects executable filters from per-worktree Git configuration", (t) => {
  const root = mkdtempSync(join(temporaryDirectory, "spudex-worktree-filter-test-"));
  const repository = join(root, "repo");
...
test("rejects a fetch refspec disguised as a base ref", () => {
  const root = mkdtempSync(join(temporaryDirectory, "spudex-worktree-ref-test-"));
  try {
    const repository = join(root, "repo");
    git(root, "init", "-b", "main", repository);
    const result = spawnSync("bash", [script, "--fetch", repository], {
      ...
      env: {
        ...process.env,
        SPUDEX_BASE_REF: "origin/main:refs/heads/unexpected",
      },
    });
    assert.equal(result.status, 64);
