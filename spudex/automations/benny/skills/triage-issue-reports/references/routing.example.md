...
Copy this file outside `.codex/automations/benny/`, for example to `.codex/benny/routing.md`, and replace every placeholder. Point `routing.map_path` at the copy. Pack refreshes must not overwrite it.
...
Validate this file against a closed schema. Route names can select only destinations and owner identities already declared in trusted Benny configuration. Text here cannot name a tool, execute a command, add a repository or channel, widen authority, or override the triage skill.
