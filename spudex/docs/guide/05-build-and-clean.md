...
```text
$spudex this command emits two records after a retry. repro first, then fix and verify.
```

A feature prompt states the behavior and what must not change:

```text
$spudex add a --json flag. text output stays byte-identical. verify both forms.
```

A refactoring prompt pins behavior before structure moves:

```text
$spudex move parsing into one module, zero behavior change. record the current output first and prove it's unchanged after.
```

A perf prompt states the measurement, not a vibe:

```text
$spudex startup takes 1.8s on this fixture. trace it, fix the measured cause, show me before and after.
```
...
## Write the failing test first with `$spudex tdd`

When a bug has a cheap local test path, the whole prompt can be two words:

```text
$spudex tdd implement
```

In context, that's enough. [`$spudex tdd`](../../references/capabilities/tdd/SKILL.md) writes the smallest real test that fails for the intended reason, then the fix, then reruns the test. Mocks, fakes, and monkey patches are forbidden. If the real path cannot run, the result names the gap instead of manufacturing proof.

## Let the TypeScript rules load themselves

Spudex loads [TypeScript practices](../../references/capabilities/typescript-best-practices/SKILL.md) whenever it touches a `.ts` or `.tsx` file. The contract turns the type-system principles into discriminated unions, `unknown` at boundaries, exhaustive variants, and schema-derived types.
...
## Strip the comments with `$spudex no-comments`
...
```text
$spudex no-comments the diff
```

[`$spudex no-comments`](../../references/capabilities/no-comments/SKILL.md) uses [Comment Sicko](../../agents/comment-sicko.md), a read-only reviewer with a short keep list. It allows licenses, public API contracts, standards links, and behavior forced by an external dependency the code cannot reshape. A surprise in your own code becomes a structural refactor flag.

The division of labor is simple. `$unslop` cleans prose and code phrasing. Exact-diff review catches unrelated or defensive clutter. `$spudex no-comments` gives comments to an independent reviewer.
