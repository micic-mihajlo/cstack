...
```text
$spudex add json output to this command. text output stays byte-identical, the json parses, both run against the sample project. show me the evidence.
```
...
For a small diff you don't fully trust, [`$spudex blast-radius`](../../references/capabilities/blast-radius/SKILL.md) finds what it could break elsewhere. It picks the one fact the change is safe because of and proves it by running code instead of writing an essay about it.
...
```text
$spudex maintain-verification-skill
```

[`$spudex maintain-verification-skill`](../../references/capabilities/maintain-verification-skill/SKILL.md) audits the generated skill: one read-only source reader per feature in parallel, then one live pass that drives every mapped feature. It ends in exactly one of three outcomes. `clean` means full coverage and nothing to publish. `changed` means proven corrections confined to the verification skill's own directory, with a PR only when you asked for one. `blocked` names the blocker. It never edits product code. If the live pass catches a product regression, it reports the regression instead of papering over it in docs.

## Open the PR

```text
$spudex open the pr. small ordered commits, evidence in the description.
```
...
```text
$spudex babysit this pr. get it green.
```
...
```text
$spudex check on pr 123. anything outstanding?
```
...
```text
$spudex land the stack.
```

The [Shipping playbook](../../playbooks/shipping.md) verifies each PR independently before it arms anything. One fresh agent per PR proves the behavior live, and the agent that judges a change is never the one that wrote it. Then Shipping lands only the contiguous verified run from the bottom. It uses Graphite when the repository already uses Graphite, and the repository's supported GitHub flow otherwise. A verified PR sitting above an unverified one waits, because merging it would pull the gap in underneath.
