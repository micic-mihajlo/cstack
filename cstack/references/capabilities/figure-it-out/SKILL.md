---
name: figure-it-out
description: "Design an auditable playbook when no narrower one fits. Use for $cstack figure-it-out, a large migration, an ambitious multipart change, or work a human reviews after stepping away."
---
...
Open a todolist whose first item is to read the Principles section of the **cstack** skill. Then add the phases below as todos.
...
Log the run through [Show me your work](../show-me-your-work/SKILL.md), with one canonical TSV row per decision and unit. Keep it local by default. Commit the trail only when the user requested a PR or repository audit artifact and reviewer confidence needs it. Prefer evidence produced by rerunnable scripts. The trail plus the diff lets the human resume without replaying the run.
