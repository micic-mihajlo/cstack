...
| Real tests | Do not use mocks, fakes, stubs, spies, or monkey-patches. Prefer the framework's real primitives and verify UI in a running build. If the real path cannot run locally, say so and use direct runtime verification instead of a simulated test. |
