---
name: cstack
description: Codex-native senior engineering execution mode for rigorous implementation, debugging, review, planning, refactoring, PR driving, and long-running work. Use when the user invokes $cstack or asks for evidence-backed engineering with deliberate subagents, simple code, natural prose, and real verification.
---

# cstack

cstack maps the upstream pstack surface to Codex app and CLI. Preserve upstream intent, but claim behavioral parity only where [the parity manifest](references/parity-manifest.md) names current evidence. Resolve Cursor-only mechanics through [Codex runtime](references/codex-runtime.md) before acting.
...
This installation exposes one Codex skill front door. It does not install 21 separate principle skills or 23 separate capability skills. An exact `$cstack principle-<slug>` request maps to the leaf named below and must load that leaf in full. An exact `$cstack <capability>` request maps to the module in the Capability index and must load its `SKILL.md` in full. This command translation preserves direct addressability inside Codex without pretending the nested modules are independently installed skills.

Read the linked leaf in full whenever it applies.

### Core

- `$cstack principle-laziness-protocol` -> [Laziness Protocol](references/principles/laziness-protocol.md). Refactoring, sizing a diff, or feeling tempted to add abstractions, layers, or signal threading. Bias to deletion and the smallest change.
- `$cstack principle-foundational-thinking` -> [Foundational Thinking](references/principles/foundational-thinking.md). Before logic. Identify core types, data structures, sequencing, and shared state.
- `$cstack principle-redesign-from-first-principles` -> [Redesign from First Principles](references/principles/redesign-from-first-principles.md). Integrating a new requirement. Design as if it had been foundational from day one.
- `$cstack principle-subtract-before-you-add` -> [Subtract Before You Add](references/principles/subtract-before-you-add.md). Sequencing an addition, refactor, or rewrite. Remove dead weight first.
- `$cstack principle-minimize-reader-load` -> [Minimize Reader Load](references/principles/minimize-reader-load.md). Shaping code that is hard to trace. Collapse one-caller wrappers and shrink hidden mutable state.
- `$cstack principle-outcome-oriented-execution` -> [Outcome-Oriented Execution](references/principles/outcome-oriented-execution.md). Rewrites and migrations with explicit phases. Converge on the target instead of preserving throwaway compatibility states.
- `$cstack principle-experience-first` -> [Experience First](references/principles/experience-first.md). Product, UX, and scope tradeoffs. Choose the user's experience over implementation convenience.
- `$cstack principle-exhaust-the-design-space` -> [Exhaust the Design Space](references/principles/exhaust-the-design-space.md). Novel interactions or architecture with no precedent. Compare two or three real prototypes before committing.
- `$cstack principle-build-the-lever` -> [Build the Lever](references/principles/build-the-lever.md). Non-trivial work. Build the smallest codemod, script, generator, or probe that does or proves the job.

### Architecture

- `$cstack principle-model-the-domain` -> [Model the Domain](references/principles/model-the-domain.md). Stateful logic, repeated shape assumptions, or branch-heavy code. Encode the domain in the right structure.
- `$cstack principle-boundary-discipline` -> [Boundary Discipline](references/principles/boundary-discipline.md). Validation, errors, framework adapters, and external input. Guard boundaries and keep business logic pure.
- `$cstack principle-type-system-discipline` -> [Type System Discipline](references/principles/type-system-discipline.md). Typed signatures and models. Make illegal states unrepresentable and parse external data at boundaries.
- `$cstack principle-make-operations-idempotent` -> [Make Operations Idempotent](references/principles/make-operations-idempotent.md). Commands and loops that can retry after crashes. Repetition must converge.
- `$cstack principle-migrate-callers-then-delete-legacy-apis` -> [Migrate Callers Then Delete Legacy APIs](references/principles/migrate-callers-then-delete-legacy-apis.md). Introducing a replacement internal API. Migrate and delete in one wave when safe.
- `$cstack principle-separate-before-serializing-shared-state` -> [Separate Before Serializing Shared State](references/principles/separate-before-serializing-shared-state.md). Concurrent actors might write one file, branch, key, or object. Eliminate the sharing first.

### Verification

- `$cstack principle-prove-it-works` -> [Prove It Works](references/principles/prove-it-works.md). Before declaring done. Exercise the real artifact, not a proxy.
- `$cstack principle-fix-root-causes` -> [Fix Root Causes](references/principles/fix-root-causes.md). Debugging. Reproduce first and trace symptoms to the first causal fault.
- `$cstack principle-sequence-verifiable-units` -> [Sequence Work into Verifiable Units](references/principles/sequence-verifiable-units.md). Migrations, sweeps, commits, and PR stacks. End every small unit with a check.

### Delegation

- `$cstack principle-guard-the-context-window` -> [Guard the Context Window](references/principles/guard-the-context-window.md). Route bulk reads and large outputs to bounded subagents. Keep evidence summaries in the main task.
- `$cstack principle-never-block-on-the-human` -> [Never Block on the Human](references/principles/never-block-on-the-human.md). Proceed on safe, reversible, in-scope work. Ask only when a missing choice materially changes the outcome or needs new authority.

### Meta

- `$cstack principle-encode-lessons-in-structure` -> [Encode Lessons in Structure](references/principles/encode-lessons-in-structure.md). A repeated instruction belongs in a lint, flag, runtime check, script, or durable skill rule.
...
- For full-mode delegation, resolve an existing `cstack-agent` thread first. Resume it with the current runtime's follow-up control instead of spawning a sibling. If no such thread exists, delegate the [cstack agent](agents/cstack-agent.md) contract. The installable custom-agent template is [agents/templates/cstack-agent.toml](agents/templates/cstack-agent.toml).
...
Persistent custom agents live under `~/.codex/agents/` or `.codex/agents/`. Create or edit them only through [setup-cstack](references/capabilities/setup-cstack/SKILL.md) or another explicit user request.
...
- `$cstack architect` -> [architect](references/capabilities/architect/SKILL.md)
- `$cstack arena` -> [arena](references/capabilities/arena/SKILL.md)
- `$cstack automate-me` -> [automate-me](references/capabilities/automate-me/SKILL.md)
- `$cstack blast-radius` -> [blast-radius](references/capabilities/blast-radius/SKILL.md)
- `$cstack bro` -> [bro](references/capabilities/bro/SKILL.md)
- `$cstack create-verification-skill` -> [create-verification-skill](references/capabilities/create-verification-skill/SKILL.md)
- `$cstack figure-it-out` -> [figure-it-out](references/capabilities/figure-it-out/SKILL.md)
- `$cstack how` -> [how](references/capabilities/how/SKILL.md)
- `$cstack interrogate` -> [interrogate](references/capabilities/interrogate/SKILL.md)
- `$cstack maintain-verification-skill` -> [maintain-verification-skill](references/capabilities/maintain-verification-skill/SKILL.md)
- `$cstack make-bot-ui` -> [make-bot-ui](references/capabilities/make-bot-ui/SKILL.md)
- `$cstack no-comments` -> [no-comments](references/capabilities/no-comments/SKILL.md)
- `$cstack recall` -> [recall](references/capabilities/recall/SKILL.md)
- `$cstack reflect` -> [reflect](references/capabilities/reflect/SKILL.md)
- `$cstack setup-cstack` -> [setup-cstack](references/capabilities/setup-cstack/SKILL.md)
- `$cstack show-me-your-work` -> [show-me-your-work](references/capabilities/show-me-your-work/SKILL.md)
- `$cstack swarm` -> [swarm](references/capabilities/swarm/SKILL.md)
- `$cstack tdd` -> [tdd](references/capabilities/tdd/SKILL.md)
- `$cstack teach` -> [teach](references/capabilities/teach/SKILL.md)
- `$cstack technical-writing` -> [technical-writing](references/capabilities/technical-writing/SKILL.md)
- `$cstack typescript-best-practices` -> [typescript-best-practices](references/capabilities/typescript-best-practices/SKILL.md)
- `$cstack unslop` -> [unslop](references/capabilities/unslop/SKILL.md)
- `$cstack why` -> [why](references/capabilities/why/SKILL.md)
