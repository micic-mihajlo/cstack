...
```text
$cstack add json output to this command. text output stays byte-identical, the json parses, both run against the sample project. show me the evidence.
```
...
For a small diff you don't fully trust, [`$cstack blast-radius`](../../references/capabilities/blast-radius/SKILL.md) finds what it could break elsewhere. It picks the one fact the change is safe because of and proves it by running code instead of writing an essay about it.
...
Once the verify skill works, a [`$cstack swarm`](../../references/capabilities/swarm/SKILL.md) can split a full pass by feature-map entry and aggregate the results.
...
```text
$cstack babysit this pr. get it green.
```
...
```text
$cstack check on pr 123. anything outstanding?
```
