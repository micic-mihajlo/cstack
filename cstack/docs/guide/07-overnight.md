...
```text
$cstack im going to bed. migrate every caller to the new parser in a fresh worktree off <base>.
...
Because you'll review this work after stepping away, `$cstack` routes it through [`$cstack figure-it-out`](../../references/capabilities/figure-it-out/SKILL.md), which designs the run's phases before any code and wires in the decision log.
...
```text
$cstack full autopilot on this queue. each item is independent. i want them merged by morning.
```
...
```text
$cstack autopilot these five changes but stack them, don't ship. i'll land the stack in the morning.
```
...
```text
$cstack orchestrate the store migration. own it until every package is converted and merged. i'll check in twice a day.
```
