...
const lockPath = join(scriptsDirectory, ".cstack-tools-install.lock");
...
function pathsFor(root: string): TreePaths {
  return {
    ...
    installKey: join(root, ".cstack-tools-install-key"),
    integrity: join(root, ".cstack-tools-integrity.json"),
  };
}
...
function currentUid(): number {
  if (process.getuid === undefined) {
    throw new Error("cstack dependency integrity requires a POSIX user identity");
  }
...
function acquireInstallLock(installKey: string): LockIdentity | null {
  ...
  while (true) {
    const temporaryLock = join(
      scriptsDirectory,
      `.cstack-tools-lock-${process.pid}-${randomBytes(8).toString("hex")}`
    );
    ...
    if (Date.now() >= deadline) {
      throw new Error("timed out waiting for the cstack dependency installer");
    }
...
function installLockIsActive(): boolean {
  try {
    ...
    if (
      ...
      throw new Error("unsafe cstack dependency-install lock");
    }
    const value = readOwnedFile(lockPath, 32).toString("utf8").trim();
    if (!/^[1-9][0-9]*$/.test(value)) {
      throw new Error("invalid cstack dependency-install lock owner");
    }
    ...
    throw new Error(
      `stale cstack dependency-install lock for dead PID ${owner}; ` +
...
function installAndPublish(installKey: string): void {
  const stage = mkdtempSync(join(scriptsDirectory, ".cstack-tools-stage-"));
  chmodSync(stage, 0o700);
  const candidate = join(stage, "node_modules");
  const backup = join(
    scriptsDirectory,
    `.cstack-tools-backup-${process.pid}-${randomBytes(8).toString("hex")}`
  );
