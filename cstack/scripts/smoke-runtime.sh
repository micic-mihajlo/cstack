...
store="$(mktemp -d "${TMPDIR:-/tmp}/cstack-orch-smoke.XXXXXX")"
...
if ! (cd "$script_dir" && bun run test) 2>&1 | tee "$test_evidence"; then
	printf 'real cstack test suite failed; evidence: %s\n' "$test_evidence" >&2
	exit 1
fi
...
safe_git -C "$fixture" config user.name "cstack Runtime Smoke"
safe_git -C "$fixture" config user.email "cstack-runtime-smoke@example.invalid"
printf '%s\n' \
	'cstack orchestration receipt fixture.' \
...
printf '%s\n' \
	...
	'  assert.equal(verificationLabel("github.com/cstack/runtime-smoke", 101, head), `github.com/cstack/runtime-smoke#101@${head}`);' \
	'});' \
	'' \
	'test("rejects a shortened head", () => {' \
	'  assert.throws(() => verificationLabel("github.com/cstack/runtime-smoke", 101, "abc"), /invalid head/);' \
	'});' \
...
fixture_repo=github.com/cstack/runtime-smoke
fixture_pr=101
