...
export type GitObjectId = string & {
  readonly [gitObjectIdBrand]: "GitObjectId";
};
...
export function parseGitObjectId(
  ...
  if (
    ...
    throw new Error(`${label} must be a full 40- or 64-hex object ID`);
...
export interface PullRequestFacts {
  ...
  readonly headRefOid: GitObjectId | null;
...
export interface CheckRead {
  readonly source: "rest-commit-gates";
  readonly headRefOid: GitObjectId;
...
export interface GitHubMergeRefusal {
  ...
  readonly reason: "merge-state" | "head-gates";
  readonly mergeStateStatus: MergeStateStatus;
  readonly headGateState: "FAILURE" | "PENDING" | "SUCCESS";
}
export interface GitHubMergeAllowed {
  readonly kind: "allowed";
  readonly basis: "merge-state-and-rest-gates";
  readonly mergeStateStatus: "CLEAN" | "DRAFT";
  readonly headGateState: "SUCCESS";
}
...
export interface GitHubReader {
  /** Absolute monotonic-seconds deadline for all subsequent command reads. */
